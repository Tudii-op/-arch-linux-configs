...
PanelWindow {
    ...
    Component { id: cSysStats;   SystemStatsModule {} }
    Component {
        id: cClock
        ClockModule {
    ...
    mask: Region {
        ...
        // Hot zone: only the top-center strip above the pill, not the whole
        // top edge of the screen.
        Region {
            x: Math.round((root.width - pill.collapsedWidth) / 2)
            y: 0
            width: root.autohide ? Math.round(pill.collapsedWidth) : 0
